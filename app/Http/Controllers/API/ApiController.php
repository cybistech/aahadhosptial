...
class ApiController extends Controller {
      ...
      $response = array("status" => 0, "msg" => "Validation error");
            ...
            if ($validator->fails()) {
                  ...
                  $response['msg'] = $message;
                     ...
                     $past=Appointment::where("user_id",$request->get("user_id"))->select("doc_id","user_id","department_id","id","name","date","time","status","phone_no",'messages','service_id')->where("date","<",date('Y-m-d'))->paginate(5);
                     $data=array();
                     if($past){
                          foreach ($past as $f) {
      ...
      $response = array("status" => 0, "msg" => "Validation error");
            ...
            if ($validator->fails()) {
                  ...
                  $response['msg'] = $message;
                     ...
                     $future=Appointment::where("user_id",$request->get("user_id"))->select("doc_id","user_id","department_id","id","name","date","time","phone_no","status",'messages','service_id')->where("date",">=",date('Y-m-d'))->orderby('id','desc')->paginate(10);
                     ...
                     if($future){
                          foreach ($future as $f) {
                                $getdata=Doctor::where("user_id",$f->doc_id)->first();
                                if($getdata){
                                     $f->doctor_id=$getdata->user_id;
                                     $f->doctor_email=$getdata->email;
                                     $f->doctor_name=$getdata->name;
                                     $departmentdata=Department::find($getdata->department_id);
                                     $f->department_name=$departmentdata->name;
                                     $f->image=asset('public/upload/doctor').'/'.$getdata->image;
   ...
   public function healthpackage(){
       ...
       foreach($data as $k){
           $dep=Department::find($k->department_id);
           $k->department_name=isset($dep)?$dep->name:"";
       }
        ...
        $response = array("status" => 0, "msg" => "Validation error");
           $rules = [
            ...
            if ($validator->fails()) {
                ...
                $img_url="";
                $getemail = User::where("email",$request->get("email"))->where("usertype",1)->where('id','!=',$request->get("user_id"))->first();
                if(empty($getemail)){
                    ...
                    if($user){
                     ...
                     $response = array("status" =>0, "msg" => "Email Already Exist");
                }
