    ...
    @foreach ($events as $index => $e)
        <div class="auto-container">
            <div class="news-block-three">
                <div class="inner-box">
                    <a href="{{ route('news_detail', ['slug' => $e->slug]) }}">
                        <div class="image-box">
                            <?php
                                if($e->featured_image){
                            ...
                            <a href="{{ route('news_detail', ['slug' => $e->slug]) }}" class="date">{{ \Carbon\Carbon::parse($e->created_at)->format('j F, Y') }}</a>
                        </div>
                    </a>
                    <div class="content-box">
                        <h4><a href="{{ route('news_detail', ['slug' => $e->slug]) }}">{{ $e->title }}</a></h4>
                        <a href="{{ route('news_detail', ['slug' => $e->slug]) }}">
